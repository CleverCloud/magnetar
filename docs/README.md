...
| [`consumer-stall-recovery.md`](consumer-stall-recovery.md) | A `Shared` subscription wedged broker-side after consumer churn (issue #414): the symptom, why the connection keepalive cannot see it, detection via `available_permits()` / `ConsumerStalled` / admin `topic_stats`, and the opt-in bounded auto-recovery → `resubscribe()` → `topics unload` recovery ladder (ADR-0101, ADR-0103).         |
