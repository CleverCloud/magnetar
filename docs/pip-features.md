...
- **End-to-end**: 2 tests at `crates/magnetar/tests/e2e_replicated_subscriptions.rs` against the two-cluster Docker fixture: cursor-resume across a cluster failover, and remote materialization of the replicated subscription once the acked position crosses a snapshot window.
  ...
  Runs as a regular `cargo test` per ADR-0046: the two-cluster fixture is brought up in the per-PR `test` job of [`ci.yml`](../.github/workflows/ci.yml), so these tests run on every PR and push alongside the rest of the e2e suite — ADR-0046 folded the former weekly `e2e-replicated-subs.yml` into per-PR CI and deleted it.
