...
| [`stream_consumer_equivalence.rs`](../crates/magnetar-differential/tests/stream_consumer_equivalence.rs)                                     | Eight baseline public scalable-aggregate scenarios: typed delivery, concurrent receive and atomic batch, aggregate budget, same-epoch assignment and drain, child reconnect, acknowledgement retry, negative-ack redelivery, and close wakeup (ADR-0098).                                           |
| [`stream_consumer_advanced_equivalence.rs`](../crates/magnetar-differential/tests/stream_consumer_advanced_equivalence.rs)                   | Six advanced public scalable-aggregate scenarios: local split/merge ancestry, the exact M1 sealed-assignment limitation, Strict versus BrokerManaged cross-member ordering, vector seek/resynchronization, transaction commit/abort/poison, and controller incarnation ordering (ADR-0098).         |
...
ADR-0098 adds the façade and fake without claiming a later record total, and ADR-0096's isolated target, output-only LCOV, artifact rejection, and cleanup behavior remain unchanged.
...
| [`e2e_scalable_topic.rs`](../crates/magnetar/tests/e2e_scalable_topic.rs)                       | Pulsar 5.0.0-M1 scalable lookup and namespace-watch compatibility plus the ADR-0098 public assignment-driven aggregate contract: typed multi-segment delivery, vector ack, broker-effective inclusive vector-seek replay, transaction commit/abort, single-member split progression in Strict mode, BrokerManaged cross-member behavior, reachable broker-authored authority matching the standalone bootstrap transport, and logical-close membership residue. Because the broker controls child assignment and the advertised endpoint equals bootstrap, this is not evidence that client-side Strict gating caused the pause, that authority selection beat fallback, or that multi-broker routing works. |
