...
| [`batch_redelivery_flow_equivalence.rs`](../crates/magnetar-differential/tests/batch_redelivery_flow_equivalence.rs)                         | Issue #436 — a `Shared` subscription over BATCHED entries with `ack_timeout` armed: the scripted broker re-dispatches a partially-acked entry carrying `CommandMessage.ack_set`, charges permits per message as `batch_size - ackedCount`, and only advances mark-delete over a contiguous run of fully-acked entries.                                                                                                                                                       |
| [`send_batch_error_equivalence.rs`](../crates/magnetar-differential/tests/send_batch_error_equivalence.rs)                                   | Error-path parity for the `Op::SendBatch` op issue #436 added: publishing a batched entry on a dropped producer, and on a connection the scripted broker's decode-fatal frame has just taken terminal, must land in exactly the same buckets the plain `Op::Send` does — on both engines (ADR-0105).                                                                                                                                                                         |
| [`dead_letter_flow_refund_equivalence.rs`](../crates/magnetar-differential/tests/dead_letter_flow_refund_equivalence.rs)                     | Issue #437 — a one-permit `Shared` consumer with a dead-letter threshold: two `Op::NackShared` rounds push the entry's `redelivery_count` past `max_redeliver_count`, and the entry published behind the dead-lettered one must still be dispatched, because a unit the broker charged is refunded at routing time (ADR-0107). Adds `max_redeliver_count` to `Op::OpenSharedConsumer`, plus `Op::NackShared` and `Op::DrainDeadLettersShared` / `Event::DeadLettersDrained`. |
...
| [`e2e_batch_ack_timeout_shared.rs`](../crates/magnetar/tests/e2e_batch_ack_timeout_shared.rs)   | Issue #436 — a `Shared` subscription over BATCHED entries with `ack_timeout` armed, against a broker pinned to `acknowledgmentAtBatchIndexLevelEnabled=true`: a consumer restart splits a batched entry, and the replacement must be handed only the positions it still owes, drain the subscription, and complete the split entry so the broker's backlog empties with no non-contiguous deleted range left (ADR-0105). |
