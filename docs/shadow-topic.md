...
This page documents the magnetar surface for shadow topics: the admin REST methods, the producer-side replicator entry, the consumer-side classification contract, and the caveats inherited from upstream.
...
The fixture is **self-hosting** — one standalone container via `testcontainers-rs`, no external broker dependency, no second cluster (PIP-180 is intra-cluster; cross-cluster geo-replication is PIP-33, covered separately by [`docs/replicated-subscriptions.md`](replicated-subscriptions.md)).
...
> That mismatch is tracked in [`docs/follow-ups.md`](follow-ups.md).
