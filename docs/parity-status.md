...
| **Athenz (ZTS round-trip)** | 🟡 scaffold landed (`feature = "auth-athenz-zts"`, default off) | `zts::ZtsClient` wraps the reqwest-backed `POST /zts/v1/oauth2/token` (or legacy `n-token`) exchange with `tokio::sync::Mutex`-guarded expiry-aware caching. JWT signing is a pluggable `zts::JwtSigner` trait — magnetar ships no concrete signer (choice is downstream policy: jsonwebtoken / aws-lc-rs / ring / HSM). `AthenzProvider::with_zts_client(...)` + `refresh_via_zts(...)` (async) primes the cache; `initial()` (sync) returns the cached role token. Remaining v0.2.0 work: a concrete `JwtSigner` impl (or document the picked external pattern), a Dockerised ZTS fixture for the e2e suite, and the matching e2e tests behind `feature = "e2e"`. |
