...
| **Athenz (ZTS round-trip)** | ✅ landed (`feature = "auth-athenz-zts"`, default off; built-in signer behind `crypto-aws-lc-rs` / `crypto-ring`) | `zts::ZtsClient` wraps the reqwest-backed `POST /zts/v1/oauth2/token` (or legacy `n-token`) exchange with `tokio::sync::Mutex`-guarded expiry-aware caching. JWT signing has two paths: (a) `jwt_signer::AwsLcRsSigner` / `jwt_signer::RingSigner` ship in-tree, gated on the workspace crypto-provider feature matrix per [ADR-0035](../specs/adr/0035-pluggable-crypto-provider.md) — mirrors the rustls provider selection so a single feature flip switches every consumer; or (b) a caller-supplied `zts::JwtSigner` impl (jsonwebtoken / HSM) via `AthenzProvider::with_zts_client(...)`. `AthenzProvider::with_default_signer(config)` constructs the cfg-active backend in one call (cfg cascade picks aws-lc-rs first). Parsed PKCS#8 DER bytes wrapped in `zeroize::Zeroizing<…>` (closes the ADR-0030 deferral note). Both backends emit byte-identical signatures per RFC 8017 §8.2; cross-backend equivalence is pinned by the `ring::tests::cross_backend_signature_byte_identity` test (gated on both features). End-to-end coverage lives in [`crates/magnetar/tests/e2e_athenz_zts.rs`](../crates/magnetar/tests/e2e_athenz_zts.rs) under `feature = "e2e,auth-athenz-zts"`: three wiremock-stub tests drive the full `ZtsClient` HTTP path (cached `initial()` after `refresh_via_zts`, expiry-driven refresh, challenge-response reuses cached token) plus a Docker reachability probe against `athenz/athenz-zts-server:1.12.41`. The full production-style ZMS+ZTS+cert-bootstrap topology is documented as deferred scope in the test file's module doc — closing it would require the consumer to ship the Athenz `make deploy-dev` 4-container stack as a shared CI fixture. |
