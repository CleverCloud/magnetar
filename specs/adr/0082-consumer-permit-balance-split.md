...
- Known bounded drift: chunks that are dropped instead of buffered (out-of-range, cap-evicted, duplicate on incomplete-message redelivery, straggler, seek-drop) do not decrement `permit_balance`, leaving it one high per occurrence even though the broker debited a permit for the dispatch. The drift only under-signals starvation to the policy (growth fires slightly later; `maybe_flow` replenishment is unaffected, so nothing stalls) and resets at the next zeroing site. Accepted as out of scope for the #349 signal fix.
