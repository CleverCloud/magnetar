...
- **A non-advancing epoch is ignored, never fatal.** `handle_update` returns `Ok(None)` and leaves the session untouched when the incoming `epoch` does not exceed the applied one. The epoch is monotonic, but nothing forbids the broker from re-sending a layout it already sent, and Pulsar 5.0.0-M1 does so as a matter of course: it answers the lookup with the current layout and then pushes that same layout, at that same epoch, on the watch the lookup just opened. This was a `DagError::NonMonotonic` until 2026-08-04, and since `Connection` closes the session on any update error, the broker's ordinary duplicate tore the watch down and blinded the client to every later epoch — the split among them. No scripted test caught it because every one of them advanced the epoch on every frame; only `e2e_scalable_topic_drops_on_broker_split` against a real broker did. Only a `SessionMismatch`, a broker-side error, or a bodyless update ends a session.
