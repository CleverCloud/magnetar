...
- **Harder**: the two engines answer "is there unflushed ciphertext?" differently, and the difference is load-bearing rather than cosmetic.
  moonpool owns its adapter, so `write_has_work` reads the real state via `TransportWriteHalf::has_pending_ciphertext()`.
  tokio cannot see through `tokio_rustls`, so it carries an explicit `flush_pending` flag threaded into `write_one_budget`.
  The flag is required, not defensive: `tokio_rustls` answers `poll_write` with `Poll::Ready(Ok(n))` while its `would_block` flag is set (the `(n, true) => Poll::Ready(Ok(n))` arm in `tokio-rustls/src/common/mod.rs`), so bytes reach the rustls session buffer while the socket write blocks, and `write_budgeted` can drain `pending_write` to empty with encrypted records still unflushed.
  A flush cancelled by another arm winning the round would then leave `write_has_work` false — arm gated off, nothing re-polling, and the stall not charged against `write_deadline` either, since it is cleared on the same branch.
  The flag is set BEFORE `write_budgeted`, not merely before the flush, because a cancellation mid-`write_budgeted` can already have handed bytes to rustls and advanced `front_offset` past them; the price is one no-op flush per round on a TLS connection.
- **Harder / residual risk**: `tokio_rustls::client::TlsStream`'s behaviour when only the read half is polled and a protocol-mandated response (KeyUpdate ack) is pending internally could not be verified from source in this change (tokio-rustls's internal buffering is opaque to this crate, unlike moonpool's home-grown adapter which this ADR gives full control over).
  ...
  Note this is a strictly narrower gap than the `flush_pending` case above, which is about ordinary write backpressure racing an inbound read and IS closed.
