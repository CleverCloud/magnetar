...
- **Status**: Accepted (amends [ADR-0088](0088-sim-coverage-gate-scope-report-ungated-additions.md) — the report scope it measured and recorded as narrow is widened; its `not gated` reporting and its fail-open analysis remain binding). Partially superseded by [ADR-0092](0092-enforce-sim-coverage-and-gate-every-pull-request.md) — **the advisory landing below is reversed**: `SIM_COVERAGE_ENFORCES_UNCOVERED` is now `true` and the check runs on every pull request. Read **every** statement below that depends on the constant being `false` as historical, without exception — including "exits 0", "nothing is newly blocked", "not evidence of ADR-0024 patch coverage", "the tracked follow-up", and the backlog-growth argument for flipping it. [ADR-0102](0102-assignment-driven-m1-hardened-stream-consumer.md) further amends the six-package closure to eight and narrows the record-less-file advisory to files with no non-test function body. The report widening, `SIM_COVERAGE_GATED_CRATE_PREFIXES`, whole-crate record-less hard failure and clang pinning remain binding.
- **Date**: 2026-07-31
