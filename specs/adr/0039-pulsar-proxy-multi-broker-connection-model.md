...
> **Amendment (2026-06-14, redirect-target dialing).** This amendment closes the redirect-CHASE gap — the last piece of multi-broker lookup that still bounced on the bootstrap — and corrects two now-inaccurate paragraphs.
>
> _What changed._ On a `CommandLookupTopicResponse` with `LookupType = Redirect`, `magnetar-proto` no longer chases the redirect on the bootstrap socket (it used to re-encode a fresh `CommandLookupTopic` on the same connection — which, on a multi-broker cluster where the bootstrap is not the bundle owner, re-asked the same non-owner broker and looped to `MAX_LOOKUP_REDIRECTS`, failing every lookup). The proto layer now surfaces a **driveable** `LookupOutcome::Redirected { broker_service_url, broker_service_url_tls, authoritative, hops_remaining }`. Both engines consume it in their lookup loop: they dial the redirect-target broker (reusing `resolve_direct_broker` / the per-broker `ProxyConnectionPool`, no new connection machinery) and re-issue the lookup THERE via the new `Connection::lookup_redirect(topic, authoritative, hops_remaining)` entry — Java `BinaryProtoLookupService#findBroker` recursing on `getConnection(redirectAddress)` (`BinaryProtoLookupService.java:182`, `:207`). When the resolved owner answers `Connect` with no advertised URL, the data ops ride the connection the lookup landed on (the dialed target), not the bootstrap (`resolve_target` now takes a `landed_on` connection).
>
> _Hop-budget threading._ The `MAX_LOOKUP_REDIRECTS` cap was previously enforced entirely inside proto (`translate_lookup_response` decrements `hops_remaining` and short-circuits to `Failed` at zero). With the chase moved to an engine-driven dial loop, `hops_remaining` is carried out to the engine on the `Redirected` outcome and threaded back into the next `lookup_redirect`, decremented per dialed broker. The cap is now enforced in three cooperating places: the proto translate-layer floor (`Failed` at zero), a proto-side clamp in `lookup_redirect` (`hops_remaining.min(MAX_LOOKUP_REDIRECTS)` so a buggy engine cannot inflate the budget and re-open the redirect-loop DoS), and an engine-side guard that refuses to dial a `Redirected` with `hops_remaining == 0` and surfaces the same synthetic `Failed { code: 0, message: "lookup redirect cap exceeded …" }`.
>
> _Corrections to this ADR's prose._ Two paragraphs below were already inaccurate at HEAD and this amendment fixes them:
>
