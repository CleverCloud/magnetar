...
| Controller subscribe through a proxy                      | [apache/pulsar#26275](https://github.com/apache/pulsar/issues/26275) | Direct controller routing is supported; missing or unrouteable controller authority and proxy-any-broker registration fail closed                                                                                                                                  |
...
Direct mode selects the authority matching the bootstrap transport, validates every broker-authored controller and segment URL through the redirect allow-list before reusing credentials, and connects through supervised pooling.
Tokio reuses its scheme-aware pool; Moonpool first gains equivalent TLS-capable supervised pool and reconnect state rather than silently downgrading or using the bootstrap connection.
If no controller URL is available, the client retries within the operation deadline and then returns a specific controller-unavailable error.
