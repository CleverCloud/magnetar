...
9. **Cross-runtime test + coverage policy.** Every behavioral change (runtime behavior, public API, wire format) and every change inside `magnetar-proto` ships with **all four** test layers in the same commit: (a) `magnetar-proto` unit test, (b) `magnetar-runtime-tokio` integration test, (c) `magnetar-runtime-moonpool` integration test, (d) `magnetar-differential` equivalence test asserting tokio ↔ moonpool `EventStream` parity, plus an end-to-end test under `crates/magnetar/tests/e2e_*.rs`.
   Patch coverage is **100% on the diff** across two isolated hard evidence domains (`cargo run -p xtask -- check-sim-coverage`): Moonpool+differential owns shared/proto/Moonpool/differential/façade/fakes/auth source, while a fresh Tokio unit/integration+differential run owns only Tokio adapter source (ADR-0103).
   The Moonpool report owns seven packages: `magnetar-proto`, `magnetar-runtime-moonpool`, `magnetar-differential`, `magnetar-auth-athenz`, `magnetar-auth-sasl`, `magnetar-driver` (directory `crates/magnetar`), and `magnetar-fakes`; the disjoint Tokio report owns only `magnetar-runtime-tokio`.
   The original six-crate widening measured 63 `SF:` records on 2026-07-31 (`magnetar-proto` 28, `magnetar-runtime-tokio` 12, `magnetar-runtime-moonpool` 12, `magnetar-auth-athenz` 5, `magnetar-differential` 4, `magnetar-auth-sasl` 2); ADR-0102 adds the façade and fakes through `magnetar-differential`'s public aggregate tests without inventing a later record total. The measurement runs at `opt-level = 0` ([ADR-0094](specs/adr/0094-measure-sim-coverage-unoptimized.md)), overriding the workspace's `[profile.test] opt-level = 1` for its duration.
   ...
   Moonpool execution still runs only `magnetar-runtime-moonpool` + `magnetar-differential`, so proto/shared lines require sim evidence; Tokio execution separately runs that crate's own unit and integration tests for adapter evidence.
   One locked, invocation-owned scratch root contains separate Moonpool and Tokio Cargo/llvm-cov targets outside cached storage; profiles, objects, profdata, and reports never cross domains, LCOV is output-only, and injected artifact flags fail closed ([ADR-0100](specs/adr/0100-isolate-sim-coverage-current-pass-artifacts.md), [ADR-0103](specs/adr/0103-isolate-moonpool-and-tokio-coverage-evidence.md)).
   **The uncovered-line verdict is enforced**: `SIM_COVERAGE_ENFORCES_UNCOVERED` is `true`, so an uncovered added line inside the reported scope is printed in full with a count and **fails** the check.
   ...
   ADR-0102's record-less-file rule remains: a gated file with no `SF:` record hard-fails when its crate is absent or it contains a production function body; module/export/constant/bodyless-declaration-only files remain advisory. Executable `unreachable!`, `unimplemented!`, and `todo!` lines are gated. Detecting a missing `DA:` mapping inside an otherwise recorded file remains unresolved; this tick makes no parser-free hard claim about it.
   The façade library and `magnetar-fakes` are now measured because `magnetar-differential` declares both as dev-dependencies and its tests exercise the public aggregate; the façade's Docker-bound `crates/magnetar/tests/e2e_*.rs` targets do not run in either domain.
