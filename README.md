...
- **PIPs implemented or partially wired**: PIP-4 (end-to-end encryption),
  ...
  PIP-37 (chunking — bounded consumer-side reassembly, cap 10 / 60s expiry, matching Java — plus redelivery backoff), PIP-54 (partial-batch ACK), PIP-87
...
| `sendTimeout`                                            | ✅   | ✅       | `ProducerBuilder::send_timeout`. Defaults to **30 s** (Java parity — `sendTimeoutMs = 30000`, ADR-0070); a send whose receipt is lost/corrupted in flight fails with a `code=-1, "send timeout"` error instead of hanging. `ProducerBuilder::disable_send_timeout` restores the unbounded behavior.                                                                                                                                                                                                                       |
