...
Magnetar's current release is `1.2.0`.
...
```toml
[dependencies]
magnetar-driver = { git = "https://github.com/CleverCloud/magnetar", tag = "v1.2.0" }
```
...
```
$ magnetarctl --version
magnetarctl 1.2.0 (a1b2c3d4e5f6)
...
| `messageListener` (push delivery)                                     | ✅   | ✅       | `ConsumerBuilder::message_listener(...)` + `subscribe_with_listener()` (and the `TypedConsumerBuilder` twin, delivering a decoded `TypedMessage<S>`). A spawned poller drives `receive()` and invokes the callback **sequentially, in order**; the callback **acks explicitly** (no auto-ack, Java parity); it stops cleanly on consumer close / handle drop. Pull and push are mutually exclusive — `subscribe_with_listener()` moves the consumer into the poller, returning a `MessageListenerHandle`. Full parity: single-topic + typed **and** the wrapper consumers — `MultiTopicsConsumerBuilder` / `PartitionedConsumerBuilder` / `PatternConsumerBuilder` each take `message_listener(...)` + `subscribe_with_listener()`, with a `Fn(&str, &IncomingMessage)` callback (the topic routes the explicit ack). Pattern / partition children discovered **after** subscribe inherit the listener (the poller races `receive()` against a membership-change signal). ADR-0064. |
...
| PIP-37                   | Chunking + ack-timeout redelivery backoff                                                  | ✅          | `magnetar-proto::producer`, `trackers::nack`; consumer-side reassembly bounded in `magnetar-proto::consumer` (`max_pending_chunked_message` cap 10, `expire_time_of_incomplete_chunked_message` 60s, `auto_ack_oldest_chunked_message_on_queue_full` false — Java-matching)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
The public API is stable as of `1.2.0` and follows Semantic Versioning; the experimental surfaces noted above (PIP-460 scalable topics, the CLI `produce`/`consume` subcommands) are excluded from that guarantee.
