...
async fn e2e_controlled_cluster_failover_manual_swap() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Consumer should also have been rebuilt against broker-B. The supervised
    // reconnect re-subscribes the consumer and re-issues the per-subscription
    // `CommandFlow` permits (deferred flow on the Subscribe ack — `conn.rs`
    // `rebuild_consumers` sets `flow_on_subscribe_ack`, the Success arm re-issues
