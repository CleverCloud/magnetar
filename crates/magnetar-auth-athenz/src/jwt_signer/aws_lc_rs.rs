...
//! aws-lc-rs RS256 backend for the Athenz [`crate::zts::JwtSigner`].
//!
//! Gated behind `feature = "crypto-aws-lc-rs"`. Pairs with the rustls
//! `aws_lc_rs` crypto provider so a single workspace feature flip
//! switches every consumer (rustls + Athenz signer + PIP-4 message
//! encryption) at once (issue #9, ADR-0035).
...
/// aws-lc-rs RSA backend. Holds the parsed PKCS#8 DER bytes under
/// [`Zeroizing`] so the secret is wiped on drop (ADR-0030 close-out).
///
/// `RsaKeyPair` itself is an opaque wrapper around an aws-lc-rs
/// `EVP_PKEY` (C-allocated) and cannot be made `Zeroize`-friendly from
/// Rust; we therefore rebuild the keypair on every [`Self::sign`] call
/// from the zeroized DER bytes. The extra cost is one PKCS#8 ASN.1
/// parse + `EVP_PKEY` rebuild per N-token mint — negligible alongside
/// the 2048-bit modular exponentiation the signature already does.
pub struct AwsLcRsSigner {
    /// PKCS#8-encoded private key DER. Wiped on drop.
    key_der: Zeroizing<Vec<u8>>,
    /// Reused PRNG. aws-lc-rs ignores the rng for PKCS#1 v1.5 sign
    /// (the encoding is deterministic per RFC 8017 §8.2) but the
    /// `RsaKeyPair::sign` signature still requires one.
    rng: SystemRandom,
}

impl std::fmt::Debug for AwsLcRsSigner {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        // Never surface key material via Debug; mirror the
        // `AthenzConfig` redaction policy.
        f.debug_struct("AwsLcRsSigner")
...
impl AwsLcRsSigner {
    ...
    ///   PRIVATE KEY-----` block).
    /// - [`AthenzError::SignerFailure`] when aws-lc-rs rejects the PKCS#8 blob (wrong algorithm
    ///   OID, modulus out of policy, …).
    ...
    pub fn from_pem_str(pem: &str) -> Result<Self, AthenzError> {
        ...
        let _ = RsaKeyPair::from_pkcs8(&key_der).map_err(|e| {
            AthenzError::SignerFailure(format!(
                "aws-lc-rs rejected the athenz tenant PKCS#8 key: {e}"
            ))
        })?;
    ...
    pub fn sign_raw(&self, signing_input: &[u8]) -> Result<Vec<u8>, AthenzError> {
        ...
        key_pair
            ...
            .map_err(|e| {
                AthenzError::SignerFailure(format!("aws-lc-rs RSA-SHA256 sign failed: {e}"))
            })?;
        Ok(signature)
    }
}

impl JwtSigner for AwsLcRsSigner {
...
mod tests {
    use aws_lc_rs::signature::{
        KeyPair, RSA_PKCS1_2048_8192_SHA256, RsaKeyPair, UnparsedPublicKey,
    };
    use base64::Engine;
    ...
    /// PKCS#8 v1 RSA-2048 test key. Generated offline with
    /// `openssl genpkey -algorithm RSA -outform PEM -pkeyopt rsa_keygen_bits:2048`
    /// + `openssl pkcs8 -topk8 -nocrypt`. No production credential.
...
-----END PRIVATE KEY-----";
    ...
    fn fixed_claims() -> ZtsClaims {
        ZtsClaims {
            ...
            kid: "key0".into(),
            // 2025-01-01T00:00:00Z; pinned for deterministic-signature
            // assertions.
            iat: 1_735_689_600,
            exp: 1_735_689_660,
        }
    ...
    fn from_pem_str_parses_pkcs8_rsa_key() {
        let signer = AwsLcRsSigner::from_pem_str(TEST_PKCS8_PEM).expect("parse PKCS#8 key");
        // Smoke: a sign call must not error and must produce a
        // non-empty signature.
    ...
    fn from_pem_str_rejects_garbage_pem() {
        let err = AwsLcRsSigner::from_pem_str(
            "-----BEGIN PRIVATE KEY-----\nnope\n-----END PRIVATE KEY-----\n",
        )
        .unwrap_err();
        assert!(
    ...
    #[test]
    fn sign_round_trips_through_aws_lc_rs_verify() {
        let signer = AwsLcRsSigner::from_pem_str(TEST_PKCS8_PEM).expect("parse PKCS#8 key");
        ...
        // Rebuild the keypair from the same PKCS#8 to extract the
        // public key for the verify check.
        ...
        verifier
            ...
            .expect("aws-lc-rs verify must accept the freshly-minted signature");
    }
    ...
    fn sign_is_deterministic_for_fixed_claims() {
        // RSASSA-PKCS1-v1_5 with SHA-256 is deterministic per RFC 8017
        // §8.2 — two sign calls with the same key + payload must
        // produce byte-identical signatures. This is the property the
        // moonpool deterministic-simulation engine relies on for
        // cross-engine reproducibility (sans-io wall_clock frozen at
        // the call site).
        let signer = AwsLcRsSigner::from_pem_str(TEST_PKCS8_PEM).expect("parse");
        let claims = fixed_claims();
    ...
    fn zeroizing_wrap_pins_drop_semantics() {
        // The signer wraps its DER bytes in `Zeroizing<Vec<u8>>`. We
        // cannot observe the wipe from safe Rust (the inner pointer
        // is freed by the time `Drop` returns), so the test pins the
        // type-level invariant: shadowing the field's type with a
        // non-`Zeroize` `Vec<u8>` would break the binding here.
        fn assert_zeroize_pinned<T: zeroize::Zeroize>(_: &T) {}
        let signer = AwsLcRsSigner::from_pem_str(TEST_PKCS8_PEM).expect("parse");
        // `Zeroizing<Vec<u8>>` deref-targets `Vec<u8>` and the inner
        // `Vec<u8>` is `Zeroize`. Compile-time check.
