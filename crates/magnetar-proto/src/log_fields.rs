...
//!
//! Broker-supplied strings (advertised redirect URLs, server error
//! messages) are hostile-peer-controlled input. Before such a string lands
//! in a structured `tracing` field it is truncated to a fixed budget — a
//! log-injection and cardinality defence mirroring sozu's render-time
//! sanitisation. This is the proto-local copy used at the point-of-detection
//! log sites (ADR-0054 §5 single-owner rule); the runtime engines carry
//! their own per-crate copies so the helper never crosses the crate
