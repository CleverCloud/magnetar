...
//! Last-clone drop guard — tokio engine (issue #241).
//!
...
//! (`crates/magnetar-runtime-moonpool/tests/producer_drop_close.rs`) so
...
async fn explicit_close_then_drop_sends_single_close_producer() {
    ...
    // Ordering barrier: this open's round-trip reaches the broker after
    // any duplicate CloseProducer the drop could have enqueued.
    let barrier = client
        .open_producer(CreateProducerRequest {
