...
//! ADR-0053 — OpenTelemetry context propagation wire-level tests (tokio engine).
//!
//! Mirror of `magnetar-runtime-moonpool/tests/otel_context_propagation.rs`.
...
//!
//! Parity required by ADR-0024 — count of tests must match the moonpool
...
fn open_producer(conn: &mut Connection, topic: &str, at: Instant) -> ProducerHandle {
    ...
    // and the ack below must correlate with it (the producer-not-ready
    // drain gate only opens on a matching `ProducerSuccess`).
