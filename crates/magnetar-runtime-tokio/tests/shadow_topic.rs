...
//! PIP-180 / ADR-0033 shadow-topic integration — tokio engine.
//!
//! Mirror of `magnetar-runtime-moonpool/tests/shadow_topic.rs`. Drives
//! `magnetar_proto::Connection` directly with synthetic broker frames so
//! the same wire trace exercises both engines (the runtime crates'
//! public `Producer::send_with_source_message_id` and
//! `Consumer::set_shadow_source` are thin delegates over the sans-io
//! methods these tests touch).
//!
