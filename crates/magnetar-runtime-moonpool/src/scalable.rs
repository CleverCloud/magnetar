...
use crate::client::{ClientError, direct_broker_authority};
use crate::consumer::subscribe_manual_flow_on;
use crate::pool::ProxyConnectionPool;
use crate::{ConnectionShared, ScalableEvent, SleepProvider};
...
#[derive(Clone)]
pub struct SegmentSubscriber<P: Providers> {
...
impl<P: Providers> std::fmt::Debug for SegmentSubscriber<P> {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("SegmentSubscriber")
...
impl<P: Providers + Send + Sync> SegmentSubscriber<P> {
    pub(crate) fn new(
        bootstrap: Arc<ConnectionShared>,
        pool: Arc<ProxyConnectionPool<P>>,
        operation_timeout: std::time::Duration,
    ...
    /// Spawn aggregate control work through the configured Moonpool provider.
    /// Dropping the opaque handle detaches; shutdown is cooperative through
    /// aggregate state and `Notify`, matching the connection driver.
    pub fn spawn_task<F>(&self, name: &'static str, future: F) -> ScalableTaskHandle<P>
    where
        F: std::future::Future<Output = ()> + Send + 'static,
    {
        self.spawn_task_with_completion(name, future, || {})
    }

    fn spawn_task_with_completion<F, C>(
        &self,
        name: &'static str,
        future: F,
        on_complete: C,
    ) -> ScalableTaskHandle<P>
    where
        F: std::future::Future<Output = ()> + Send + 'static,
        C: FnOnce() + Send + 'static,
    {
        ...
        let join = self.pool.task_provider().spawn_task(name, async move {
            future.await;
            drop(completion);
        });
    ...
    pub async fn sleep(&self, duration: std::time::Duration) -> Result<(), ClientError> {
        (self.sleep_provider)(duration)
            .await
            .map_err(|error| ClientError::Other(format!("scalable retry sleep failed: {error}")))
    }
    ...
    ) -> Result<StreamConsumer<P>, StreamConsumerError> {
    ...
    pub async fn open_dag_session(&self, topic: &str) -> Result<DagSession, ClientError> {
        ...
        let initial = match self
            .next_with_timeout(pending.route(), "scalable DAG lookup")
            .await
        {
            Ok(event) => event,
    ...
    pub async fn open_control_plane(
        ...
        moonpool_core::select! {
            ...
            _ = &mut timeout => Err(ClientError::ControllerUnavailable),
        }
    }
    ...
    async fn open_controller_session_with_id(
        ...
        let controller_url = if self.pool.uses_tls() {
        ...
        let baseline = self
            .controller_baseline_with_timeout(pending.route(), incarnation)
            .await?;
    ...
    pub async fn open_segment_consumer(
    ...
    ) -> Result<crate::Consumer<P>, ClientError> {
        ...
        let broker_url = if self.pool.uses_tls() {
            descriptor.broker_url_tls.as_deref()
        } else {
            descriptor.broker_url.as_deref()
        }
    ...
    async fn resolve_direct_url(
        ...
        let expected_tls = self.pool.uses_tls();
        match (
            expected_tls,
            magnetar_proto::broker_endpoint_scheme(broker_url),
        ) {
            (false, Some(magnetar_proto::BrokerEndpointScheme::PulsarTls))
            | (true, Some(magnetar_proto::BrokerEndpointScheme::Pulsar))
            | (_, None) => {
                return Err(ClientError::ControllerRoutingUnsupported {
        ...
        let physical = direct_broker_authority(broker_url, self.pool.schemeless_default_port())?;
        if physical == self.pool.bootstrap_addr() {
            return Ok(self.bootstrap.clone());
        }
        Ok(crate::pool::get_or_open(self.pool.clone(), broker_url, &physical, None, 0).await?)
    }
    ...
    ) -> Result<ScalableEvent, ClientError> {
        let next = route.next();
        let sleep = (self.sleep_provider)(self.operation_timeout);
        let mut next = std::pin::pin!(next);
        let mut sleep = std::pin::pin!(sleep);
        moonpool_core::select! {
            biased;
            result = &mut next => result.map_err(ClientError::from),
            _ = &mut sleep => Err(ClientError::Other(format!(
                "{operation} exceeded operation_timeout"
            ))),
        }
    }

    async fn controller_baseline_with_timeout(
        &self,
        route: &ScalableRoute,
        ...
        let sleep = (self.sleep_provider)(self.operation_timeout);
        let mut baseline = std::pin::pin!(baseline);
        let mut sleep = std::pin::pin!(sleep);
        moonpool_core::select! {
            biased;
...
#[derive(Clone)]
struct ChildRuntime<P: Providers> {
    source: magnetar_proto::SegmentSource,
...
struct AggregateState<P: Providers> {
    model: magnetar_proto::StreamConsumerModel,
    receive: magnetar_proto::StreamReceiveState,
    children: BTreeMap<magnetar_proto::SegmentId, ChildRuntime<P>>,
...
impl<P: Providers> AggregateState<P> {
    fn new(
        model: magnetar_proto::StreamConsumerModel,
...
struct AcknowledgementCancellation<'a, P: Providers> {
    inner: Weak<StreamConsumerInner<P>>,
    authority: &'a magnetar_proto::AcknowledgementAuthority,
    armed: bool,
}

struct SeekCancellation<P: Providers> {
    inner: Weak<StreamConsumerInner<P>>,
    armed: bool,
}

impl<P: Providers + Send + Sync + 'static> SeekCancellation<P> {
    fn new(inner: &Arc<StreamConsumerInner<P>>) -> Self {
        Self {
...
impl<P: Providers + Send + Sync + 'static> Drop for SeekCancellation<P> {
    fn drop(&mut self) {
        if self.armed
...
impl<'a, P: Providers + Send + Sync + 'static> AcknowledgementCancellation<'a, P> {
    fn new(
        inner: &Arc<StreamConsumerInner<P>>,
        authority: &'a magnetar_proto::AcknowledgementAuthority,
    ) -> Self {
        Self {
            inner: Arc::downgrade(inner),
...
impl<P: Providers + Send + Sync + 'static> Drop for AcknowledgementCancellation<'_, P> {
    fn drop(&mut self) {
        if self.armed
            && let Some(inner) = self.inner.upgrade()
        {
...
struct TransactionAcknowledgementCancellation<P: Providers> {
    inner: Weak<StreamConsumerInner<P>>,
    authority: Option<magnetar_proto::TransactionAcknowledgementAuthority>,
}

impl<P: Providers + Send + Sync + 'static> TransactionAcknowledgementCancellation<P> {
    fn new(
        inner: &Arc<StreamConsumerInner<P>>,
...
impl<P: Providers + Send + Sync + 'static> Drop for TransactionAcknowledgementCancellation<P> {
    fn drop(&mut self) {
...
struct TransactionRegistrationCancellation<P: Providers> {
    inner: Weak<StreamConsumerInner<P>>,
    key: (magnetar_proto::TxnId, magnetar_proto::SegmentSource),
    armed: bool,
}

impl<P: Providers + Send + Sync + 'static> TransactionRegistrationCancellation<P> {
    fn new(
        inner: &Arc<StreamConsumerInner<P>>,
        key: (magnetar_proto::TxnId, magnetar_proto::SegmentSource),
    ) -> Self {
...
impl<P: Providers + Send + Sync + 'static> Drop for TransactionRegistrationCancellation<P> {
    fn drop(&mut self) {
        if self.armed
            && let Some(inner) = self.inner.upgrade()
        {
...
struct StreamConsumerInner<P: Providers> {
    subscriber: SegmentSubscriber<P>,
    child_options: SegmentConsumerOptions,
    topic: String,
    subscription: String,
    consumer_name: String,
    consumer_id: u64,
    state: Mutex<AggregateState<P>>,
    notify: Notify,
    #[cfg(test)]
...
impl<P: Providers> std::fmt::Debug for StreamConsumerInner<P> {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("StreamConsumerInner")
...
/// Provider-native aggregate over assigned ordinary child consumers.
pub struct StreamConsumer<P: Providers + Send + Sync + 'static> {
    inner: Arc<StreamConsumerInner<P>>,
}

impl<P: Providers + Send + Sync + 'static> std::fmt::Debug for StreamConsumer<P> {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
...
impl<P: Providers + Send + Sync + 'static> StreamConsumer<P> {
    async fn open(
        subscriber: SegmentSubscriber<P>,
        options: StreamConsumerOptions,
    ...
    pub async fn receive_batch(
        ...
        let reserve = self.inner.reserve_batch(max_messages, max_bytes);
        let sleep = (self.inner.subscriber.sleep_provider)(max_wait);
        let mut reserve = std::pin::pin!(reserve);
        let mut sleep = std::pin::pin!(sleep);
        moonpool_core::select! {
            biased;
            result = &mut reserve => result,
            _ = &mut sleep => Ok(Vec::new()),
        }
    }
...
impl<P: Providers + Send + Sync + 'static> Drop for StreamConsumer<P> {
    fn drop(&mut self) {
        self.inner.close_best_effort();
    }
}

impl<P: Providers + Send + Sync + 'static> StreamConsumerInner<P> {
    fn track_task(state: &mut AggregateState<P>, handle: ScalableTaskHandle<P>) {
    ...
    ) -> Result<crate::Consumer<P>, StreamConsumerError> {
    ...
    async fn finish_cancelled_open(
        ...
        source: magnetar_proto::SegmentSource,
        child_generation: magnetar_proto::ChildGeneration,
        consumer: Option<crate::Consumer<P>>,
    ) -> Result<(), StreamConsumerError> {
    ...
    async fn reopen_dag_watch(
        ...
        SegmentSubscriber::<P>::align_control_plane(&mut replacement, controller).await?;
    ...
    async fn control_loop(self: Arc<Self>, mut dag: DagSession, mut controller: ControllerSession) {
        loop {
            ...
            let update = {
                let dag_next = dag.next();
                let assignment_next = controller.next_assignment();
                let mut dag_next = std::pin::pin!(dag_next);
                let mut assignment_next = std::pin::pin!(assignment_next);
                moonpool_core::select! {
                    biased;
                    () = &mut notified => ControlUpdate::Wake,
                    result = &mut dag_next => ControlUpdate::Dag(result),
                    result = &mut assignment_next => ControlUpdate::Assignment(result),
                }
            };
            match update {
    ...
    async fn reconnect_control_plane(
        ...
        loop {
                ...
                .await
            {
                Ok(mut controller) => {
                    let aligned = {
                        let alignment = SegmentSubscriber::<P>::align_control_plane(
                            &mut opened_dag,
                            &mut controller,
                        );
                        let mut alignment = std::pin::pin!(alignment);
                        let closing = self.wait_for_close();
...
impl<P: Providers> std::fmt::Debug for ScalableTaskHandle<P> {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("ScalableTaskHandle")
            .finish_non_exhaustive()
    }
}

impl<P: Providers> ScalableTaskHandle<P> {
...
mod tests {
    ...
    fn subscriber_with_allow_list() -> SegmentSubscriber<moonpool_core::TokioProviders> {
        ...
        let pool = ProxyConnectionPool::new(crate::pool::ConnectionFactory {
            addr: "allowed.example:6650".to_owned(),
    ...
    fn aggregate_inner_with_two_messages() -> Arc<StreamConsumerInner<moonpool_core::TokioProviders>>
    {
    ...
    fn empty_aggregate_inner() -> Arc<StreamConsumerInner<moonpool_core::TokioProviders>> {
        let subscriber = subscriber_with_allow_list();
        let (snapshot, assignment) = control_plane_fixture();
        let mut model = magnetar_proto::StreamConsumerModel::new(
            "topic://public/default/scaled".to_owned(),
    ...
    fn attached_child_consumer(
        ...
        crate::Consumer<moonpool_core::TokioProviders>,
        ...
        (
            crate::Consumer::<moonpool_core::TokioProviders>::assemble(
                shared.clone(),
                handle,
                slot,
                None,
    ...
    async fn scalable_task_handle_joins_owned_work() {
        let subscriber = subscriber_with_allow_list();
        let handle = subscriber.spawn_task("scalable-test", async {});
    ...
    async fn completed_aggregate_tasks_are_reaped_when_tracking_new_work() {
        let inner = empty_aggregate_inner();
        let completed = inner.subscriber.spawn_task("completed", async {});
        tokio::time::timeout(std::time::Duration::from_secs(1), async {
        ...
        let pending = inner.subscriber.spawn_task("pending", async move {
    ...
    async fn segment_subscribe_starts_paused_without_initial_flow() {
        ...
        let task = tokio::spawn(async move {
            subscribe_manual_flow_on::<moonpool_core::TokioProviders>(
                task_shared,
                request,
                std::time::Duration::from_secs(1),
                crate::tokio_sleep_provider(),
    ...
    async fn concurrent_close_waits_for_owned_task_barrier() {
        ...
        let handle = inner
            .subscriber
            .spawn_task("scalable-close-test", async move {
                task_gate.notified().await;
            });
