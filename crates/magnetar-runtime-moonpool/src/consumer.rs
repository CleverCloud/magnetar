...
impl<P: Providers> Consumer<P> {
    ...
    /// — i.e. messages it has authorised the broker to push and the broker has
    /// not yet spent. Returns `0` for closed/unknown handles. Mirrors Java
    /// `ConsumerBase#getAvailablePermits`. 1:1 with the tokio engine.
    ///
    ...
    /// could not detect issue #414's silent Shared-subscription stall. See
...
mod tests {
    ...
    async fn resubscribe_reattaches_in_place_and_refuses_an_ineligible_consumer() {
        ...
        let orphan: Consumer<TokioProviders> =
            make_consumer(shared, magnetar_proto::ConsumerHandle(9999));
        assert!(
