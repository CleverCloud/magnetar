...
//! must re-attach the producer in place — moonpool engine twin of
//! `crates/magnetar-runtime-tokio/tests/producer_broker_close_reattach.rs`
...
//! keeps serving. No `Connection::reset` ever runs, so `rebuild_producers` never
//! fires; before ADR-0106 the slot stayed at `broker_ready = false` for the life
//! of the connection and every `send()` routed to it resolved
//! `code=-1 "send timeout"` until the process restarted.
//!
//! The recovery lives entirely in the sans-io `magnetar_proto::Connection`, so
//! this twin also gives the ADR-0024 `check-sim-coverage` gate its reach into
//! the new `conn.rs` lines (only the moonpool and differential binaries are
//! measured).
...
use magnetar_proto::{
    ...
    RequestId, decode_one, encode_command, pb,
};
use magnetar_runtime_moonpool::{Client, ConnectionShared, MoonpoolEngine};
...
/// Bind a broker and return its `host:port` (the moonpool engine's
/// `connect_plain` takes a bare address, not a `pulsar://` URL).
        ...
        .run_until(async {
            ...
            let producer = tokio::time::timeout(
                ...
                client.open_producer(CreateProducerRequest {
                    topic: "persistent://public/default/reattach-451-moonpool".to_owned(),
            ...
            tokio::time::timeout(HANG_GUARD, producer.send(outgoing(b"first")))
                .await
                .expect("first send did not time out")
                .expect("first send ok");

            // The broker has now closed the producer on the live socket. This
            // send is the one that used to fail `code=-1 "send timeout"` forever.
            tokio::time::timeout(
                Duration::from_secs(10),
                producer.send(outgoing(b"after-close")),
            )
            .await
            .expect(
                "send WEDGED after a broker CommandCloseProducer on a live connection: the \
            ...
            assert_eq!(
                *log.producer_opens.lock(),
                vec![None, Some(1)],
                "the broker must see the initial open (no epoch) plus exactly one in-place \
                 re-attach at epoch 1"
            );
            assert_eq!(
    ...
    local
        .run_until(async {
            ...
            let producer = tokio::time::timeout(
                HANG_GUARD,
                client.open_producer(CreateProducerRequest {
                    topic: "persistent://public/default/reattach-451-moonpool-retry".to_owned(),
            ...
            tokio::time::timeout(HANG_GUARD, producer.send(outgoing(b"after-close")))
                .await
                .expect(
                    "send WEDGED: the ServiceNotReady-rejected re-attach never rode the \
                     operation-retry leg (issue #451)",
                )
            ...
            assert_eq!(
                *log.producer_opens.lock(),
                ...
                "initial open, the rejected re-attach at epoch 1, then the retry leg's \
                 re-issue at epoch 2"
            );
            assert_eq!(
                log.accepts.load(Ordering::SeqCst),
...
fn close_during_in_flight_open_still_fails_the_open() {
    ...
    let handle = conn.create_producer(CreateProducerRequest {
        topic: "persistent://public/default/reattach-451-moonpool-in-flight".to_owned(),
        ..Default::default()
    });
