...
//! Moonpool sibling of the tokio `stats_interval_sweep` test (ADR-0024 1:1
//! runtime-test parity) — ADR-0089 layer (c).
//!
...
//!
//! Moonpool territory: expressing the tick as a deadline rather than a task is
//! what makes it representable here at all. An interval-driven ticker would
//! need `tokio::time` — which ADR-0078 lists as incompatible with the
//! `SimProviders` executor — whereas this sweep rides the same `poll_timeout`
//! wake the simulated clock already schedules, so it is bit-for-bit
//! reproducible under a fixed `MOONPOOL_SEED`. The disabled-by-default arm
//! below is the other half of that claim: a `None` interval must contribute no
//! deadline at all, because an armed-but-never-firing one would still perturb
//! the simulated wake schedule.
//!
...
//! would deadlock here rather than in review). No listener, no wall clock, no
//! Docker: every instant is synthetic, so the published rates are exact.
//!
//! Tokio sibling: `crates/magnetar-runtime-tokio/tests/stats_interval_sweep.rs`.
