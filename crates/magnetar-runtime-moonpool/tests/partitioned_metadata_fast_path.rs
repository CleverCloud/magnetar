...
//! `magnetar-runtime-tokio/tests/partitioned_metadata_fast_path.rs`.
//!
//! F11 fast-path verification at the moonpool engine layer: when the
...
//!
//! Strategy mirrors the tokio sibling: stand up a tiny in-process TCP
//! broker stub that records the order of every `BaseCommand` it sees,
//! drive the moonpool engine through `partitioned_topic_metadata` with
//! a `-partition-0` topic, then assert the recorded sequence contains
//! NO `PartitionedMetadata` frame. The control case (non-partition
//! topic) MUST observe one `PartitionedMetadata` frame.
    ...
    local
        .run_until(async {
            ...
            let count = tokio::time::timeout(
                ...
                client
                    .partitioned_topic_metadata("persistent://public/default/foo-partition-0"),
            )
            .await
...
/// `PartitionedMetadata` frame to the broker.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
