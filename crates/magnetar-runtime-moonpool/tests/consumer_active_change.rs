...
//! `crates/magnetar-runtime-tokio/tests/consumer_active_change.rs`.
//!
//! Runs the moonpool engine over `TokioProviders` against a real loopback
//! `TcpListener` (the `logging_checksum.rs` / `reconnect_replay_gating.rs`
//! harness pattern) so both new `ActiveChangeFut` implementations get real
//! test coverage — a `ConnectionShared`-only test (locking `.inner` and
//! driving `handle_bytes` directly, the shape `ack_orphan_close.rs`'s
//! moonpool twin uses) would never touch this engine's
//! `is_active()` / `next_active_change()` code at all, since those are
//! façade-level futures, not proto-level state. Identical scenario names and
//! count to the tokio file keeps `cargo xtask check-runtime-test-parity` 1:1
//! (ADR-0024).
//!
//! Scenario 3 (`next_active_change_manual_poll_covers_repark_and_drop_cancel`)
//! adds a manual `Future::poll` drive of the private `ActiveChangeFut`
...
fn emit_connected(out: &mut BytesMut) {
    let cmd = pb::BaseCommand {
        ...
        connected: Some(pb::CommandConnected {
            server_version: "magnetar-consumer-active-change-moonpool".to_owned(),
...
/// Broker session for scenario 1: answers CONNECT / LOOKUP / SUBSCRIBE
/// normally, piggy-backing an immediate promotion (`is_active: true`) on the
/// subscribe ack. On `demote_trigger`, pushes a standalone demotion
...
async fn spawn_promote_demote_broker() -> (String, Arc<Notify>) {
    ...
    (addr.to_string(), demote_trigger)
}

/// Broker session for scenario 2: answers CONNECT / LOOKUP / SUBSCRIBE /
/// `CLOSE_CONSUMER` normally; never emits an `ActiveConsumerChange` frame.
...
async fn promote_and_demote_resolve_via_next_active_change() {
    let (addr, demote_trigger) = spawn_promote_demote_broker().await;
    ...
    let consumer = tokio::time::timeout(
        ...
        client.subscribe(SubscribeRequest {
            topic: "persistent://public/default/consumer-active-change-mp".to_owned(),
            subscription: "consumer-active-change-mp".to_owned(),
...
async fn next_active_change_resolves_err_after_close() {
    let addr = spawn_close_only_broker().await;
    let engine = MoonpoolEngine::new(TokioProviders::new());
    let client = tokio::time::timeout(
        HANG_GUARD,
        Client::connect_plain(&engine, &addr, ConnectionConfig::default()),
    ...
    let consumer = tokio::time::timeout(
        HANG_GUARD,
        client.subscribe(SubscribeRequest {
            topic: "persistent://public/default/consumer-active-change-close-mp".to_owned(),
            subscription: "consumer-active-change-close-mp".to_owned(),
            sub_type: pb::command_subscribe::SubType::Failover,
            receiver_queue_size: 16,
            durable: true,
            ..Default::default()
...
async fn next_active_change_manual_poll_covers_repark_and_drop_cancel() {
    ...
    let consumer = tokio::time::timeout(
        HANG_GUARD,
        client.subscribe(SubscribeRequest {
            topic: "persistent://public/default/consumer-active-change-manual-poll-mp".to_owned(),
            subscription: "consumer-active-change-manual-poll-mp".to_owned(),
            sub_type: pb::command_subscribe::SubType::Failover,
            receiver_queue_size: 16,
            durable: true,
            ..Default::default()
