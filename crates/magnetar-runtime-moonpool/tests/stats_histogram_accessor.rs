...
//! different layer: they drive a real `Consumer<MoonpoolEngine<..>>` /
//! `Producer<MoonpoolEngine<..>>` over a loopback broker and call the
//! *runtime facade* accessors (`Consumer::receive_latency_histogram` /
//! `Producer::send_latency_histogram`, the one-line `self.slot.state.lock().
...
//! on the host clock, so they stay count-only.
...
/// drives a real `Consumer<MoonpoolEngine<TokioProviders>>` over a loopback
/// broker and calls `Consumer::receive_latency_histogram` (consumer.rs) — the
/// runtime facade delegation the `ConnectionShared`-level test above never
/// touches.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn consumer_receive_latency_histogram_accessor_reflects_popped_messages_via_live_facade() {
    let addr = spawn_single_message_broker().await;
    ...
    let consumer = tokio::time::timeout(
        ...
        client.subscribe(SubscribeRequest {
            topic: "persistent://public/default/stats-hist-accessor-live-mp".to_owned(),
            subscription: "stats-hist-accessor-live-mp".to_owned(),
            sub_type: pb::command_subscribe::SubType::Shared,
...
/// drives a real `Producer<MoonpoolEngine<TokioProviders>>` over a loopback
/// broker and calls `Producer::send_latency_histogram` (producer.rs) — the
/// runtime facade delegation the `ConnectionShared`-level test above never
/// touches.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn producer_send_latency_histogram_accessor_reflects_receipts_via_live_facade() {
    let addr = spawn_producer_receipt_broker().await;
    let engine = MoonpoolEngine::new(TokioProviders::new());
    let client = tokio::time::timeout(
        HANG_GUARD,
        Client::connect_plain(&engine, &addr, ConnectionConfig::default()),
    ...
    let producer = tokio::time::timeout(
        ...
        client.open_producer(CreateProducerRequest {
            topic: "persistent://public/default/stats-hist-accessor-live-producer-mp".to_owned(),
            ..Default::default()
        }),
    )
    .await
