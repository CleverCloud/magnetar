...
//! ADR-0054 checksum log-capture test — moonpool engine, real loopback
//! broker (ADR-0024 layer c for the proto checksum point-of-detection
//! diff).
//!
//! 1:1 twin of `crates/magnetar-runtime-tokio/tests/logging_checksum.rs`
//! (ADR-0024 runtime-test-parity). Drives the moonpool engine through a
//! plain `CommandConnect` → `CommandConnected` handshake against a scripted
//! loopback broker, after which the broker writes ONE CRC32C-corrupted
//! frame raw to the stream (construction per the proto unit test
//! `frame::tests::detects_crc32c_mismatch`). With a **global** TRACE-level
//! capturing subscriber installed, the test asserts:
//!
...
//! whole run (`moonpool-sim` `runner/builder.rs`), which shadows any global
//! capturing subscriber for every event emitted inside the simulation — the
//! capture sink would stay empty. Driving the engine over `TokioProviders`
//! against a real loopback `TcpListener` (the `tests/lookup_before_open.rs`
//! harness pattern) exercises the same moonpool engine code paths
//! (`handshake_plain`, driver event pump, silent `ChecksumMismatch` drain)
//! with no subscriber shadowing.
//!
//! # Why this file is its own integration-test binary with ONE test fn
//!
//! The proto `error!` fires on the engine's read path, which may run on a
//! detached driver task, so the capturing subscriber must be **global**
...
async fn checksum_mismatch_logged_once_and_connection_survives() {
    ...
    let client = tokio::time::timeout(
        ...
        Client::connect_plain(&engine, &addr.to_string(), ConnectionConfig::default()),
    )
    .await
