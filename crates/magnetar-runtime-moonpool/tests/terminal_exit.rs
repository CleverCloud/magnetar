...
//! Plain-connection terminal fail-fast — moonpool engine, real loopback
//! broker (ADR-0024 layer c for the ADR-0055 §1 terminal-fail-fast fix). 1:1
//! twin of `crates/magnetar-runtime-tokio/tests/terminal_exit.rs`
//! (ADR-0024 runtime-test-parity).
//!
//! This is the engine where the regression actually bit: a bit-flip on a
//! command frame killed the plain driver and left every parked
//! `subscribe()` / `send()` future hanging — the moonpool no-progress stall
//! that surfaced the swizzle-clog seed-replay failure. The fix
//! (`Connection::fail_all_pending`) resolves every pending op terminally so
//! the futures return promptly.
//!
//! The moonpool engine runs over [`moonpool_core::TokioProviders`] against a
//! real loopback `TcpListener` (the `tests/reconnect_replay_gating.rs`
//! harness pattern — the moonpool surface is what diverges from tokio, not
//! the transport). Script:
//!
//! 1. connect → `CommandConnected`; lookup → use-current; producer open → `CommandProducerSuccess`;
//! 2. the client issues `subscribe()` and `send()` concurrently; on the FIRST data-plane frame the
//!    broker sees after the producer open, it drops the socket WITHOUT acking — a terminal peer
//!    close;
//! 3. both in-flight futures must resolve with a terminal `ClientError::PeerClosed` PROMPTLY (the
//!    `tokio::time::timeout` wrappers enforce "no hang").
//!
//! `connect_plain` is the UNSUPERVISED entry (driver exits on the first I/O
//! failure → plain terminal-exit path), the connection shape ADR-0055 §1
//! hardens.
//!
...
//! 1:1 twin of the tokio engine's same-named test.
...
async fn run_terminal_broker(listener: TcpListener, data_plane_seen: Arc<AtomicBool>) {
    ...
    loop {
        ...
        if drop_now {
            // Drop the stream → terminal peer close (read returns 0) on the
            // client's plain driver.
            return;
        }
...
async fn plain_connection_in_flight_ops_fail_fast_on_terminal_drop() {
    ...
    // Default config = NO supervisor. `connect_plain` is the UNSUPERVISED
    // entry: the driver exits on the first I/O failure → plain terminal-exit
    // path (no reconnect). This is the connection shape ADR-0055 §1 hardens.
    ...
    // first of these data-plane frames; both in-flight futures must resolve
    // with a terminal error PROMPTLY (the timeout enforces "no hang" — the
    // regression was an infinite park).
...
/// doomed pending op no driver is left to resolve. New-op companion to the
/// in-flight contract above (ADR-0055 §1).
///
...
/// tight `timeout` wrappers are the no-hang guard. 1:1 twin of the tokio
