...
const SURVIVING_PRODUCER_NAME: &str = "pinned-406-surviving";
...
/// The harder interleaving, and the one that reproduced against a real Pulsar
/// 4.0.4 broker in CI after the cancel-time close landed: the broker consumes
/// the close while the producer creation is still pending, acks it, stops
/// mapping that producer id, and completes the registration anyway. No close
/// can reach that registration afterwards.
///
/// Recovery is two-phase and must play out identically on both engines: the
/// first `ProducerBusy` re-closes the abandoned id, and — the name still being
/// busy — the next attempt re-attaches under that id as a strict successor,
/// which the broker accepts. Both legs must reach the same verdict with the
/// same wire traffic.
#[tokio::test(flavor = "current_thread")]
async fn producer_busy_from_a_surviving_registration_is_recovered_on_both_engines() {
    let trace = Trace::new(
        "persistent://public/default/producer-open-surviving-equiv",
        "sub-open-surviving",
        vec![
            // Withheld success → abandoned on the deadline. The close is
            // consumed mid-creation, so the registration outlives it.
            Op::OpenNamedProducer {
                name: SURVIVING_PRODUCER_NAME.to_owned(),
            },
            // Same name: reachable only through the two-phase recovery.
            Op::OpenNamedProducer {
                name: SURVIVING_PRODUCER_NAME.to_owned(),
            },
        ],
    ...
    let broker = ScriptedBroker::bind().await.expect("broker bind");
    broker.withhold_first_producer_success_surviving_close(SURVIVING_PRODUCER_NAME);
    let pulsar_url = broker.pulsar_url();
    let host_port = broker.host_port();
    ...
    assert_eq!(
        tokio_stream, moonpool_stream,
        "engine event streams diverged for the surviving-registration recovery",
    );
    assert_eq!(
        tokio_stream.events[0],
        Event::NamedProducerOpenError {
    ...
    assert_eq!(
        tokio_stream.events[1],
        Event::NamedProducerOpened,
        "op 1 must reclaim a registration that outlived its close",
    );

    assert_eq!(
        close_producer_count(&tokio_frames),
        2,
        "tokio leg: one close from the cancellation, one from the ProducerBusy recovery, frames: {tokio_frames:?}",
    );
    assert_eq!(
        close_producer_count(&moonpool_frames),
        2,
        "moonpool leg: one close from the cancellation, one from the ProducerBusy recovery, frames: {moonpool_frames:?}",
    );
    assert_eq!(
        tokio_frames, moonpool_frames,
        "engine frame sequences diverged for the surviving-registration recovery",
    );
