...
Its only effect is one `warn!` plus one `ConnectionEvent::ConsumerStalled`; recovery is the caller's explicit `Connection::resubscribe_consumer_in_place`, escalating to an operator-side `topics unload`.
That recovery is two phases since [ADR-0108](specs/adr/0108-close-then-resubscribe-for-in-place-consumer-recovery.md): `CommandCloseConsumer` for the consumer id, then — on its `Success` alone — the permit-mirror zeroing, the issue #346 ack sweep, and the `CommandSubscribe` whose own `Success` releases the initial `CommandFlow`.
The close is what makes the re-attach real: `ServerCnx.handleSubscribe` answers a `CommandSubscribe` naming a consumer id already live on the connection with a bare `sendSuccessResponse(requestId)`, touching neither the dispatcher nor `availablePermits`, so the issue #307 emit half is correct only where the broker has already closed the consumer.
`Failover` and non-durable subscriptions are refused outright, in `resubscribe_consumer_in_place` and not in the shared eligibility gate, because for them the close is an election or a discarded cursor rather than a repair.
...
A refused attempt spends no budget and mutates nothing, whether the refusal comes from the shared eligibility gate or from the standby pre-check that runs ahead of it — a consumer the broker has reported as a Failover standby (`is_active == Some(false)`, issue #348) is reported and then left alone, because its silence is the broker behaving correctly and it never receives the dispatch unit that would give the budget back. Exhausting the budget logs one `warn!` naming `pulsar-admin topics unload` and then goes quiet, because the last attempt was also the last thing that re-armed the window.
...
One attempt is permit-NEUTRAL on the broker's aggregate counter since ADR-0108 — the close returns this consumer's remaining permits and the re-subscribe's grant takes them back — so attempts do not accumulate toward repairing a dispatcher-wide corruption, and that remains an operator's `topics unload`.
The bound is therefore a small number of chances for this consumer's own slot to come back, not a countdown; ADR-0103's "one attempt credits the aggregate back exactly `receiver_queue_size`" described the over-commit ADR-0108 removed.
...
It is also read on the way IN: a `CommandMessage` carrying `ack_set` is a re-dispatch of a partially-acked batched entry, and the bitset names the positions that still owe an acknowledgement.
`ConsumerState::deliver` explodes only those, seeds a vacant tracker entry from them, AND-accumulates them into an existing one, and charges a permit for none of the rest — the broker debited `batch_size - ackedCount` when it re-dispatched, so a mirror that debited the whole entry would drift below the broker's real `availablePermits` on every redelivery ([ADR-0105](specs/adr/0105-read-the-delivered-batch-index-ack-set.md), issue #436).
...
| PIP-54                   | Partial-batch ACK (ack_set bitset)       | ✅     | `crates/magnetar-proto/src/consumer.rs:109-130`; ack stamping: `crates/magnetar-proto/src/conn.rs:1775`; delivered-`ack_set` skip + tracker seed on the receive side: `ConsumerState::deliver` ([ADR-0105](specs/adr/0105-read-the-delivered-batch-index-ack-set.md))                                                                                                                                                                                                                                                                                                                                                                                       |
